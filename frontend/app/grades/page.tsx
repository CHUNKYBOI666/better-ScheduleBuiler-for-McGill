...
export default function GradesPage() {
  ...
  const calculateFinalGrade = (courseId: number) => {
    ...
    if (totalWeight === 0 || totalWeight !== 100) {
      alert("Please ensure parameter weights add up to 100%")
      return
    }

    const hasAllGrades = parameters.every((p) => p.grade !== undefined && p.grade >= 0)
    if (!hasAllGrades) {
      alert("Please enter grades for all parameters")
      return
    }
                          ...
                          <div className="relative p-5 rounded-2xl bg-gradient-to-br from-background/90 via-background/80 to-muted/20 border-2 border-border/40 hover:border-border/60 transition-all duration-300 hover:shadow-xl backdrop-blur-xl">
                            <div className="flex items-center gap-3 mb-5">
                              ...
                              <Button
                                size="icon"
                                variant="ghost"
                                className="h-11 w-11 flex-shrink-0 hover:bg-destructive/10 hover:text-destructive transition-all duration-200 rounded-xl opacity-0 group-hover/param:opacity-100 hover:scale-110 active:scale-95"
